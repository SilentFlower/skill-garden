...
argument-hint: [project-path] [--idea <path>] [--target auto|windows|macos|linux] [--dry-run] [--no-verify]
---
...
```bash
python .claude/skills/open-idea/scripts/open_idea.py .
```

打开指定目录：

```bash
python .claude/skills/open-idea/scripts/open_idea.py /path/to/project
```

指定 IDEA 可执行文件：

```bash
python .claude/skills/open-idea/scripts/open_idea.py /path/to/project --idea "/path/to/idea"
```

只打印将要执行的命令，不真正启动：

```bash
python .claude/skills/open-idea/scripts/open_idea.py /path/to/project --dry-run
```

发起启动但不等待项目窗口验证：

```bash
python .claude/skills/open-idea/scripts/open_idea.py /path/to/project --no-verify
```

## 斜杠命令参数

如果用户通过 `/open-idea` 传入参数，直接把 `$ARGUMENTS` 透传给脚本：

```bash
python .claude/skills/open-idea/scripts/open_idea.py $ARGUMENTS
```

如果 `$ARGUMENTS` 为空，脚本会打开当前工作目录。
...
```bash
python .claude/skills/open-idea/scripts/open_idea.py . --target linux
```
