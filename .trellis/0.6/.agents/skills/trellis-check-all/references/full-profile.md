...
所有发现候选按 `references/optional-findings.md` 先判定 `CHK-*` / `OPT-*`，再只为 `CHK-*` 分配严重度。历史 P1 若只有假设后果、没有当前错误证据，必须重新经过同一准入判断；P2 也不能自动降级。
...
- 无 `CHK-*`、无阻塞、无部分验证、无实质剩余风险；允许存在满足准入条件并已完整展示的 `OPT-*`。
