...
description: "统一 Check-All 入口：确认范围与运行上下文，按 requested/effective depth 选择 light/full profile，执行三件套落地、实现假设、完整性与规范审查；区分阻断性 CHK 与非阻断 OPT，低风险文档漂移可自动修复。触发：检查、轻量检查、全面检查、提交前检查、check-all、从 PRD/三件套到代码过一遍。"
---
# Check All 统一入口
...
4. 先按当前契约和可达证据把发现分为阻断性 `CHK-*` 与非阻断 `OPT-*`，再只为 `CHK-*` 分配 P0/P1/P2；低风险文档漂移进入 `DOC-*`。
...
4. 总是读 `references/optional-findings.md`，用于区分 `CHK-*` 与 `OPT-*`。
...
3. **分类先于严重度**：读取 `references/optional-findings.md`；当前错误、契约违背和失败验证进入 `CHK-*`，只有契约已满足且不影响当前验收的额外兜底进入 `OPT-*`。历史 P1/P2 不能直接决定处置。
4. **修改前只确认一次**：除 `DOC-*` 自动修复外，全部检查结束后通过统一报告让用户选择 `CHK-*` 修复范围；`OPT-*` 只有精确 ID 或明确的全部可选项授权才进入修复。
...
- `subagent check-all`：subagent 只做 audit-only 检查，返回结构化 `CHK-*`、`OPT-*`、`DOC-*` 候选、`check_profile` 和验证证据；主会话负责应用允许的 `DOC-*` 修复、展示报告、询问一次阻断问题修复范围和协调后续修复。
...
按 profile 检查三个维度，并执行 `references/optional-findings.md` 的分类顺序。阻断问题进入 `CHK-*`，合规的额外兜底进入 `OPT-*`，低风险文档漂移候选进入 `DOC-*`。同一根因合并，不因数量多而静默省略。
...
- 先按假设后果打 P1/P2，再用严重度决定必修或可选。
- 把失败验证、契约违背、真实可达风险或证据不足的问题降级为 `OPT-*`。
- subagent 直接修改工作区。
