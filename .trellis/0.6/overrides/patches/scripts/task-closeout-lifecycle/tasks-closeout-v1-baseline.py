...
def is_closed_task(task: TaskInfo, tasks_dir: Path) -> bool:
    ...
    if _is_physical_archive(task, tasks_dir):
        return True
    closeout = task.raw.get("closeout")
    return isinstance(closeout, dict) and closeout.get("status") == "closed"
...
def task_closeout_view(task: TaskInfo, tasks_dir: Path) -> dict:
    ...
    if isinstance(closeout, dict):
        return closeout
    if _is_physical_archive(task, tasks_dir):
        return {"status": "closed", "closedAt": None, "blockers": [], "historical": True}
    return {"status": "pending", "closedAt": None, "blockers": [], "legacy": True}
