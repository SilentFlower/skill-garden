...
- A `completed` task remains discoverable only while Close is pending or blocked. Enter the `trellis-push` completed-task preflight when publication needs recovery; otherwise report `closeout.blockers` and retry deterministic Close after the blocker is resolved. Rework requires an explicit reopen before implementation, and material scope changes require refreshed planning artifacts and Brief approval.
