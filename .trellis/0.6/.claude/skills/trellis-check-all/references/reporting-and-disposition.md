...
每个独立根因使用固定字段。`ID`、`严重度`、`来源`、`处置`、`标题` 内联在条目首行，其余字段逐行展开：

| 字段 | 呈现 | 规则 |
| --- | --- | --- |
| ID | 首行 | 首次记录时依次分配 `CHK-001`、`CHK-002`；当前修复/重检循环中不重新编号 |
| 严重度 | 首行 | `P0` 数据破坏/安全事故/无法安全继续；`P1` 功能错误/需求违背/发布阻塞；`P2` 测试/规范/维护性/非阻塞风险 |
| 来源 | 首行 | prd/design/implement/spec/assumption/verification |
| 处置 | 首行 | 仅 `已接受风险` 时追加 `` `[已接受风险]` `` 标签；待处理是默认状态，不加标签也不占行 |
| 标题 | 首行 | 描述根因，不用症状堆叠 |
| 证据 | 展开 | 同一根因的全部受影响 `file:line`，以及实际契约或命令结果；受影响位置与问题证明合并在本字段，不再单列「位置」 |
| 影响 | 展开 | 用户、数据或工程影响 |
| 建议 | 展开 | 推荐修复方式，不在检查阶段执行 |
| 验证 | 展开 | 修复后的命令或手动验证步骤 |

### `FBK-*` 兜底问题

每个满足严格准入条件的独立兜底根因使用固定字段。`ID`、`严重度`、`来源`、`处置`、`标题` 内联在条目首行，其余字段逐行展开：

| 字段 | 呈现 | 规则 |
| --- | --- | --- |
| ID | 首行 | 首次记录时依次分配 `FBK-001`、`FBK-002`；当前修复/重检循环中不重新编号 |
| 严重度 | 首行 | 与 `CHK-*` 使用同一 P0/P1/P2 影响尺度 |
| 来源 | 首行 | prd/design/implement/spec/assumption/verification |
| 处置 | 首行 | 仅 `已接受风险` 时追加 `` `[已接受风险]` `` 标签；待处理是默认状态，不加标签也不占行 |
| 标题 | 首行 | 描述具体保护路径根因，不写泛化“增强健壮性” |
| 证据 | 展开 | 保护缺失、错误或可绕过的全部受影响 `file:line`，以及实际契约或命令结果；同时承载 `references/fallback-findings.md` 准入 1「具体位置」与准入 3「问题证据」，不再单列「位置」 |
| 兜底场景 | 展开 | 可达的异常、失败、越权、数据损害或诊断盲区场景；承载准入 2「可达场景」 |
| 影响 | 展开 | 当前缺口的用户、数据、安全或工程影响 |
| 保护收益 | 展开 | 修复后恢复或新增的明确保护结果；承载准入 4「保护收益」，不得省略或并入建议 |
| 建议 | 展开 | 推荐修复方式，不在检查阶段执行 |
| 验证 | 展开 | 修复后的测试、故障注入、命令或手动验证步骤；承载准入 5「验证方式」 |

`FBK-*` 字段与 `references/fallback-findings.md` 的严格准入五项一一对应。合并「位置」到「证据」只改变呈现方式，不放宽任何准入门槛；任一准入项缺失时仍不得生成 `FBK-*`。

`CHK-*` 与 `FBK-*` 分开编号。同一根因的多个位置合并到一个问题；报告按严重度排序，但不得因此重排已经分配的 ID。新根因使用对应通道的下一个 ID。每个问题的处置状态默认为待处理且不加标签，只有 `已接受风险` 才在条目首行追加 `` `[已接受风险]` `` 标签。`仅保留报告` 不改变处置状态，相关问题仍不加标签。处置状态不改变 ID、通道和严重度。
...
3. 接受只绑定当前问题证据与实际 diff。受影响代码、契约、验证结果、问题内容或严重度变化后，原接受立即失效，问题恢复为待处理并移除首行 `` `[已接受风险]` `` 标签。
4. 已接受问题继续完整展示证据、影响、建议和验证，并在条目首行追加 `` `[已接受风险]` `` 标签；checkbox 仍保持 `- [ ]`，不得改用 `- [x]`、删除条目、改列 `DOC-*` 或伪报已修复。
...
- [ ] `CHK-001` `[P1]` `[<来源>]` <标题>
  - 证据：<全部受影响 file:line / 契约 / 命令结果>
  - 影响：<影响>
  - 建议：<修复建议>
  - 验证：<验证命令或步骤>

- [ ] `CHK-002` `[P2]` `[<来源>]` `[已接受风险]` <标题>
  - 证据：<全部受影响 file:line / 契约 / 命令结果>
  - 影响：<影响>
  - 建议：<修复建议>
  - 验证：<验证命令或步骤>
...
- 报告头部的“工作/范围/画像/结论”与“修复批次”内的行必须使用 `- ` 列表项。这些行在 Markdown 段落内会被折叠成一段，不得改回裸段落行，也不得依赖行尾空格实现换行。
- 每个问题的 `ID`、`严重度`、`来源`、`标题` 内联在条目首行，固定顺序为 `` `<ID>` `[<严重度>]` `[<来源>]` <标题> ``。仅 `已接受风险` 的问题在来源标签后追加 `` `[已接受风险]` ``；待处理不加标签。`来源` 与 `处置` 都不再单独占行。
- 已接受风险的条目 checkbox 保持 `- [ ]`。`- [x]` 保留给修复循环表达“已修复”，不得用来表示风险接受。
- 相邻问题条目之间留一个空行，条目内部的字段子项之间不留空行。该空行只保证源码可读性：Claude Code 等渲染器会把松散列表压平，不要指望它产生视觉分隔；问题之间的辨识依靠首行的 ID、严重度与来源标签。该规则同时适用于“主路径问题”与“兜底问题”区。
- 每个问题的受影响位置合并进 `证据`，不再单列「位置」行；`证据` 必须给出全部受影响 `file:line`，不得只写概括性描述。
