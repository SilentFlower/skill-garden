...
先按 `references/optional-findings.md` 判定 `CHK-*` / `OPT-*`，再只为 `CHK-*` 分配 P0/P1/P2。严重度描述已确认问题的实际影响，不决定发现是否必修。
...
每个满足全部可选准入条件的独立改进使用固定字段：

| 字段 | 规则 |
| --- | --- |
| ID | 首次记录时依次分配 `OPT-001`、`OPT-002`；当前修复/重检循环中不重新编号 |
| 标题 | 描述具体的额外兜底或 defense-in-depth 收益 |
| 来源 | spec/assumption/verification |
| 证据 | 当前契约已满足的证据、`file:line` 或命令结果 |
| 为什么可选 | 说明当前没有错误、失败验证或发布风险，不修复也不改变验收结论 |
| 收益 | 修复后增加的防御性、容错性或可观测性收益 |
| 位置 | 同一改进的全部受影响位置 |
| 验证 | 可选修复后的命令或手动验证步骤 |

`OPT-*` 不使用 P0/P1/P2，也不占用 `CHK-*` 编号。历史 P1 若只有假设后果、没有当前错误证据，必须重新经过可选准入判断；P2 也不能自动降级。证据不足时保留为 `CHK-*`、部分验证或剩余风险。
...
[<通过/未通过/阻塞>] <N> 个维度 · CHK <N> · OPT <N> · 自动修复 DOC <N> · P0 <N> / P1 <N> / P2 <N> · 验证 <通过>/<总数>
...
> 以下 `OPT-*` 不影响当前通过或发布结论，不会被 `修复全部` 隐式选中。

- [ ] `OPT-001` `[可选修复]` <标题>
  - 来源：<来源>
  - 证据：<当前契约已满足的证据 / file:line / 命令结果>
  - 为什么可选：<当前不构成错误或发布风险的理由>
  - 收益：<额外 defense-in-depth 收益>
  - 位置：<全部受影响位置>
  - 验证：<验证命令或步骤>

可选项处理：`修复 OPT-001,OPT-003`、`修复全部可选项`；不处理可直接继续。
...
操作：`修复全部`（仅 CHK）、`修复 CHK-001,CHK-003`、`仅保留报告`；需要时可追加精确 `OPT-*` ID
...
- 没有 `OPT-*` 时省略“可选改进”区。
- 没有 `CHK-*` 时省略“问题清单”“修复批次”和 CHK 操作行；如果存在 `OPT-*` 或自动修复，仍展示对应区和验证。
- 有 `CHK-*` 时只在报告末尾提供一次修复范围选择，不再逐项提问。
- `修复全部` 始终只覆盖 `CHK-*`；`OPT-*` 只有精确 ID 或明确的“修复全部可选项”授权才进入修复。
- 只有 `OPT-*` 时总体状态为通过，主动作仍是继续完成链；可选处理说明不能替代或阻断该主动作。
- interactive 标准报告必须以“下一步”段结束；停止等待不等于省略引导。
- 独立 `CHK-*` 或 `OPT-*` 不得因数量多而静默省略；先合并同根因重复项，再完整列出剩余项。
...
用户选择 `CHK-*` 修复范围，或明确选择精确 `OPT-*` / 全部可选项后：
...
5. 原 `CHK-*` / `OPT-*` 沿用各自 ID；新根因在对应通道继续递增编号。上次 `effective_depth=full` 时，本次最小深度为 full。
...
[<完成/部分完成/失败>] CHK 修复 <完成>/<计划> · OPT 修复 <完成>/<计划> · DOC 自动修复 <N> · 验证 <通过>/<总数> · 剩余 CHK <N> · 剩余 OPT <N>

| 问题 | 修复 | 验证 |
| --- | --- | --- |
| CHK-001 | <已修复/未修复/阻塞> | <通过/失败/未执行> |
| OPT-001 | <已修复/未修复/保留> | <通过/失败/未执行> |
...
检查通过后的动作由下方 `Interactive Post-Check Stop Gate` 判断：普通交互停止等待，符合 direct Git 严格通过条件时同轮进入 Phase 3.3 `trellis-update-spec`，再到 Phase 3.4 `trellis-push`。仍有 `CHK-*` 时停留在修复/重检循环；只剩 `OPT-*` 时仍按通过处理。

untracked helper 不记录 Check-All 证据。普通严格通过但尚未继续时保持 `stage=check`；只有 direct Git 同轮继续或用户后续明确继续时才 `advance --stage spec`。有剩余 `CHK-*`、部分验证、阻塞或报告后的新编辑时，先 `advance --stage implement` 再返回实现；只有 `OPT-*` 时不回到 `implement`。
...
- 无剩余 `CHK-*`：`record --result ok --effective-check-depth <light|full> --check-depth-reason <summary>`，摘要包含自动修复数量以及 `OPT-*` 数量和 ID；只有 `OPT-*` 时不进入 fix/recheck，也不消耗预算。
...
subagent 只返回结构化 `CHK-*`、`OPT-*`、`DOC-*` 候选、报告和 `check_profile`；主会话收到后必须完成允许的 `DOC-*` 处理，再完成匹配 action 的 `record + next`。
...
2. direct Git 只有在 Check-All 整体结论通过、剩余 `CHK-*` 为 0、无阻塞、无部分验证、无待用户接受的实质剩余风险时才算严格通过。允许存在合规 `OPT-*` 和已成功验证的 `DOC-*` 自动修复；标准报告输出后，同一轮进入 Phase 3.3 `trellis-update-spec`；`no-op|written` 再由其加载 `trellis-push`，`needs-review` 停止。
3. 剩余 `CHK-*`、blocked、部分验证或实质剩余风险均不满足条件：输出标准报告并停止，不运行 Update-Spec，也不生成 Git 计划。原始 Git 请求不授权自动修复普通问题、忽略问题或扩大 Git 权限。
...
4. 无 direct Git intent 且严格通过：提示用户回复 `继续`，下一轮进入 `trellis-update-spec`，再由 `trellis-push` 生成提交计划；存在 `OPT-*` 时附一句非阻断说明，用户也可改为回复精确 `OPT-*` ID，但主动作仍是 `继续`。
...
- 各维度状态、问题数和问题清单；
- `OPT-*` 可选改进及其“为什么可选”和收益；
- `DOC-*` 自动修复内容和验证；
- 已执行验证及结果；
- 未覆盖验证和剩余风险；
- 总体结论；
- 与当前结论匹配的唯一主动作引导；有 `CHK-*` 时是一次修复范围选择，部分验证/阻塞时是补充决策或验证，通过时是 Phase 3.3 / Phase 3.4 指向。`OPT-*` 只能作为非阻断的次要处理说明。
