...
每个满足严格准入条件的独立兜底根因使用固定字段。`ID`、`严重度`、`来源`、`处置`、`标题` 内联在条目的四级标题行，其余字段以加粗标签的列表项逐行展开：
...
| 证据 | 加粗字段 | 保护缺失、错误或可绕过的全部受影响 `file:line`，以及实际契约或命令结果；同时承载 `references/fallback-findings.md` 准入 1「具体位置」与准入 3「问题证据」，不再单列「位置」 |
| 兜底场景 | 加粗字段 | 可达的异常、失败、越权、数据损害或诊断盲区场景；承载准入 2「可达场景」 |
| 影响 | 加粗字段 | 当前缺口的用户、数据、安全或工程影响 |
| 保护收益 | 加粗字段 | 修复后恢复或新增的明确保护结果；承载准入 4「保护收益」，不得省略或并入建议 |
| 建议 | 加粗字段 | 推荐修复方式，不在检查阶段执行 |
| 验证 | 加粗字段 | 修复后的测试、故障注入、命令或手动验证步骤；承载准入 5「验证方式」 |
