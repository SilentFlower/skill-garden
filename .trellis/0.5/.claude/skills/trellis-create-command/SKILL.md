...
| 形态 | 何时选 | 触发方式 |
|------|--------|---------|
| **command** | 显式动作、高风险、需确认点（如 push、re-implement、create-prd、finish-work） | `/trellis:<name>` |
...
**agents 版 frontmatter**：

```yaml
---
name: <name>
description: "<简短中文描述，不影响路由>"
---
```
...
**不变量（必须遵守）**：每个 trellis 入口都成对存在 —— `.claude/<commands 或 skills>/...` 主副本 + `.agents/skills/<name>/SKILL.md` 镜像副本。漏写任一份会破坏 skill-garden install.sh 的对称分发。scope 含 skill-garden 时，这对副本也要在 `<skill-garden>/.trellis/0.5/` 下各写一份（共 4 份）。
...
| `<target>/.agents/skills/<name>/SKILL.md` | `<name>`（镜像，body 同主副本） |
| `<skill-garden>/.trellis/0.5/.claude/skills/trellis-<name>/SKILL.md` | `trellis-<name>` |
| `<skill-garden>/.trellis/0.5/.agents/skills/<name>/SKILL.md` | `<name>` |
...
| `<target>/.agents/skills/<name>/SKILL.md` | 带 frontmatter，name=`<name>`（镜像） |
| `<skill-garden>/.trellis/0.5/.claude/commands/trellis/<name>.md` | 无 frontmatter |
| `<skill-garden>/.trellis/0.5/.agents/skills/<name>/SKILL.md` | 带 frontmatter |

**同步技巧**：写完主版（`.claude/skills/trellis-<X>/SKILL.md` 或 `.claude/commands/trellis/<X>.md`）后，用 `cp + sed` 派生其他副本：

```bash
# skill 版 → agents 版
cp <target>/.claude/skills/trellis-<X>/SKILL.md <target>/.agents/skills/<X>/SKILL.md
sed -i 's/^name: trellis-<X>$/name: <X>/' <target>/.agents/skills/<X>/SKILL.md

# 对 skill-garden 同步（scope 为 skill-garden 时）
cp -r <target>/.claude/skills/trellis-<X> <skill-garden>/.trellis/0.5/.claude/skills/
cp -r <target>/.agents/skills/<X> <skill-garden>/.trellis/0.5/.agents/skills/
```
...
- ❌ 只写 `.claude/...`，漏了 `.agents/skills/<X>/`（skill-garden install 会不对称）
