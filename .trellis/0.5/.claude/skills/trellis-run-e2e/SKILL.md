...
description: "Run end-to-end behavior verification by combining a browser driver (UI), an HTTP client (API), and a DB client (assertions). Input is a 场景-路径-期望 scenario table; executes each row, logs pass/fail, and restores test data when feasible. Triggers: 「跑E2E」「端到端验证」「自动化测试场景」「跑场景测试」「e2e test」. Not for lint/unit/spec-only (use trellis-check)."
---
