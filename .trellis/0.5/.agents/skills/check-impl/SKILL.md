...
> **Note**: 这是 **post-implementation** 安全网，与 `trellis-check` 互补。
> trellis-check 覆盖结构完整性与规范性，本命令检查假设正确性。
