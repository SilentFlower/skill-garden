...
|------|------|------------------|
| 仅对齐修复 | 保持 command 形态，只修过时引用（push / re-implement 这类显式动作） | — |
...
# (3-4) 更新 agents 副本（正文与 skill 版一致，frontmatter name=<name>）
# — 用最新已对齐的内容覆盖写入
...
rm -rf <skill-garden>/.trellis/0.5/.agents/skills/<name>/
...
**body 正文必须 4 份完全一致**，只 frontmatter 的 name / description 差异。复用 `sed` 同步：

```bash
cp <target>/.claude/skills/trellis-<X>/SKILL.md <agents_target>/SKILL.md
sed -i 's/^name: trellis-<X>$/name: <X>/' <agents_target>/SKILL.md
```
...
| 2 | `<target>/.agents/skills/<name>/SKILL.md` | agents 版，trellis agent 系统读取（保留） |
