---
name: check-all
description: "全维度代码检查 — 依次执行 check-prd-impl → check-impl → trellis-check，一次性完成正确性、假设验证、完整性+规范性三维检查"
---
# Check All — 全维度代码检查
...
| 3 | `trellis-check` | 改全了没 + 写得规范吗 | git diff 影响范围 + spec 开发规范 |
...
### Step 3: 跨层完整性与代码规范检查（trellis-check）

按 `.agents/skills/trellis-check/SKILL.md` 执行。

**本轮重点**：变更涉及的所有层、所有引用点是否都同步更新了？代码是否符合项目 spec 中的编码规范？lint 和 typecheck 是否通过？
