...
Handle `discuss` and `inspect` silently. For non-destructive `direct_edit`, state once that work will use the session-scoped untracked flow, then run its `begin` command; run `prepare-edit` before any file write.
